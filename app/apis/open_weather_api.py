...
class CityWeatherFromWeatherService(Resource):
    ...
    def get(self, city):
        ...
        - Collect city received in request.
        - Find lat and lon of city
...
class CityWeatherFromWeatherServiceOrDb(Resource):
    ...
    - We check if data is already available in databse
    ...
    def get(self, city):
        ...
        if city_weather:
            forecast = Forecast.query.filter_by(city_id=city_weather.id, date=today).first()
            if forecast:
                return jsonify(
                    {
